...
**Your Options Must Be Respected** 是一个面向整合包的 NeoForge 工具模组。它会在游戏启动早期扫描整合包提供的默认文件，并把它们复制到真实的游戏目录或 `config` 目录中，适合用来分发默认按键、画面、音量、辅助功能设置，以及其他只应在首次启动时写入的配置文件。

本模组是 **Your Options Shall Be Respected** 的分支版本，在保留默认配置同步思路的基础上，针对 `options.txt` 被启动器提前创建的场景进行了调整。

## 特性

- 在 Minecraft 读取 `options.txt` 和常规配置文件前执行
- 支持分发默认 `options.txt`
- 支持分发默认 `config` 文件
- 不会反复覆盖玩家已经生成或修改过的常规配置
- 对 `options.txt` 使用标记文件，避免后续启动持续重置玩家设置
- 通过临时文件和移动操作写入，降低半写入文件的风险
- 无游戏内容、方块、物品或 UI 改动，专注于整合包默认配置同步
